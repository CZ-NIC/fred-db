...
<?cs /if ?><?cs if:changes.contact.disclose_name ?>Jméno / Name: <?cs call:print_value_bool(which, changes.contact.disclose_name, "veřejné / public", "skryté / hidden") ?>
<?cs /if ?><?cs if:changes.contact.disclose_org ?>Organizaci / Organization: <?cs call:print_value_bool(which, changes.contact.disclose_org, "veřejná / public", "skrytá / hidden") ?>
<?cs /if ?><?cs if:changes.contact.disclose_email ?>Email / Email: <?cs call:print_value_bool(which, changes.contact.disclose_email, "veřejný / public", "skrytý / hidden") ?>
<?cs /if ?><?cs if:changes.contact.disclose_notify_email ?>Notifikační email / Notify email: <?cs call:print_value_bool(which, changes.contact.disclose_notify_email, "veřejný / public", "skrytý / hidden") ?>
<?cs /if ?><?cs if:changes.contact.disclose_ident ?>Identifikační údaj / Identification data: <?cs call:print_value_bool(which, changes.contact.disclose_ident, "veřejný / public", "skrytý / hidden") ?>
<?cs /if ?><?cs if:changes.contact.disclose_vat ?>DIĆ / VAT number: <?cs call:print_value_bool(which, changes.contact.disclose_vat, "veřejné / public", "skryté / hidden") ?>
<?cs /if ?><?cs if:changes.contact.disclose_telephone ?>Telefon / Telephone: <?cs call:print_value_bool(which, changes.contact.disclose_telephone, "veřejný / public", "skrytý / hidden") ?>
<?cs /if ?><?cs if:changes.contact.disclose_fax ?>Fax / Fax: <?cs call:print_value_bool(which, changes.contact.disclose_fax, "veřejný / public", "skrytý / hidden") ?>
...
<?cs /if ?><?cs if:changes.keyset.ds.1 ?><?cs call:print_value_list(which, changes.keyset.ds, "DS záznam / DS record") ?>
<?cs /if ?><?cs if:changes.keyset.dnskey.1 ?><?cs call:print_value_list(which, changes.keyset.dnskey, "DNS klíče / DNS keys") ?>
<?cs /if ?><?cs /if ?><?cs /def ?>
=====================================================================
...
<?cs else ?>
Všechny hodnoty zůstaly beze změn.
<?cs /if ?>
