...
class Progress(object):
    ...
    @property
    def rows_total(self):
        return self.rows_todo + self.rows_done
...
def run():
    try:
        ...
        parser = argparse.ArgumentParser(description="Add uuid column to `files` table without full table lock (new uuid values are done by chunk updates)")
        ...
        add_uuid_column_impl(
            dsn, 'files', 2000, logging.getLogger('table::files'),
