...
SELECT CASE WHEN length($1::text) = 0 THEN FALSE ELSE -- bool_and() returns NULL if given 0 rows from unnest, so for empty fqdn it have to be FALSE
...
CREATE TABLE Host (
        ...
        CONSTRAINT domain_name_check CHECK (domain_name_syntax_check(fqdn))
        );
